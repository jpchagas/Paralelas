...
int main (int argc, char* argv[]){
  ...
  */
  ...
  int vector[VECTOR_SIZE], cvector[VECTOR_SIZE], fvector[VECTOR_SIZE];
  int rank, 
      nproc, 
      tam_vetor,
      metade,
...
fsize = tam_vetor * PERCENT;
memcpy(fvector,vector,(fsize*sizeof(int)));
tam_vetor = tam_vetor - fsize;
...
MPI_Send(&vector[fsize], metade, MPI_INT, 2 * rank + 1, 0, MPI_COMM_WORLD);
MPI_Send(&vector[metade+2], metade, MPI_INT, 2 * rank + 2, 0, MPI_COMM_WORLD);

bubble_sort(fvector,fsize);
...
// INTERCALA O VETOR INTEIRO PARA COLOCAR EM ORDEM
    ...
    memcpy(&vector[tam_vetor+1],fvector,(fsize*sizeof(int)));
    tam_vetor = tam_vetor + fsize + 1;
